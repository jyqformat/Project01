...
public class Circle extends JFrame {
    final int MAX = 100; // (1~1000)流星的个数
    final int SLEEP = 5; // 流星飞行的速度（数值越大，速度越慢）
    final int COLORLV = 5; // (2~20)色阶（可改变光晕大小）
    final String COLOR = null; // ("#000000"~"#ffffff")光晕颜色（如果不填或null，则为默认颜色）
    final int SIZE = 6; // (2~50)流星大小
    ...
    class Meteor { // 流星类
        ...
        public void reset() {
            int rand = (int) (Math.random() * 10000);  //随机生成流星出现位置
            if (rand > 35) {
                x = (int) (Math.random() * 6000);
                y = 0;
            } else {
                y = (int) (Math.random() * 1500);
                x = 0;
            }
            mx = (int) (Math.random() * 2 - 2);  //随机生成下落速度和角度
